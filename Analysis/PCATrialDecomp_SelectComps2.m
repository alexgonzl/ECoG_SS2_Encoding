...
% VERSION 2 uses DataBins2 to obtain fewer time bins.

% reduces the data to -1 to 1;
data.AnalysisBins2 = [0 1 1 1 1 0]==1;

out                 = [];
out.nSubjs 			= size(data.BinERPs,1);
out.nComps 			= opts.nComps;
out.nChans 			= data.nChans;
out.nFeat           = data.nBands*sum(data.AnalysisBins2);
...
for ss=1:out.nSubjs
    ...
    x = permute(data.BinERPs2{ss}(:,:,:,data.AnalysisBins2),[2 3 1 4]);
